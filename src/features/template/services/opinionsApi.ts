...
export const opinionsApi = {
  ...
  // Create new opinion
  createOpinion: async (opinionData: OpinionPayload): Promise<OpinionResponse> => {
    // Add default values for required fields
    const payload = {
      ...opinionData,
      is_featured: opinionData.is_featured !== undefined ? opinionData.is_featured : false,
      is_approved: opinionData.is_approved !== undefined ? opinionData.is_approved : true,
    };
    
    const response = await api.post("opinions", payload);
    return response.data;
  },

  // Update existing opinion
  updateOpinion: async (id: string, opinionData: OpinionPayload): Promise<OpinionResponse> => {
    // Add default values for required fields
    const payload = {
      ...opinionData,
      is_featured: opinionData.is_featured !== undefined ? opinionData.is_featured : false,
