...
interface CertificateTableProps {
  ...
  onTableReady?: (table: ReactTable<CourseCertificate>) => void;
...
const columns: ColumnDef<CourseCertificate>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const certificate = row.original;
      return <span className="font-medium">{certificate.issuedCount}</span>;
    },
  },
  {
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground mt-1 text-right">
                    {row.getValue("issuedCount")} من{" "}
                    {row.getValue("totalStudents")} طالب
                  </p>
                </div>
