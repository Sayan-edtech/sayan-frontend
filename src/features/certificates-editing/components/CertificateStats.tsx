...
import { List, Users, BarChart3, TrendingUp, Award } from "lucide-react";
...
function CertificateStats({ certificates }: CertificateStatsProps) {
  const stats = useMemo(() => {
    ...
    const totalIssued = certificates.reduce(
      (sum, cert) => sum + cert.issuedCount,
      0
    );
    const averageCompletion =
      certificates.length > 0 ? totalCompletion / certificates.length : 0;
