...
export function LessonsList({ chapters, onToggleChapter, onSelectLesson }: LessonsListProps) {
  const getLessonIcon = (type: Lesson['type']) => {
    switch(type) {
      case 'video': return <Video className="w-4 h-4 text-red-500" />;
      case 'quiz': return <ClipboardCheck className="w-4 h-4 text-blue-500" />;
      case 'interactive': return <Zap className="w-4 h-4 text-purple-500" />;
            ...
            <div key={chapter.id} className="space-y-2">
              ...
              >
                ...
                <div className="text-right">
                  ...
                  <div className="text-xs text-gray-500">{chapter.lessons.length} دروس</div>
                </div>
              </button>
                ...
                <div className="mr-6 space-y-1">
                  {chapter.lessons.map((lesson) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2">
                        <div className={`text-sm font-medium ${lesson.isActive ? 'text-blue-900' : 'text-gray-900'}`}>
                          {lesson.title}
                        </div>
