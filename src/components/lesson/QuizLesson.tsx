...
const questions: Question[] = [
  {
    ...
    options: ['تنسيق الصفحات', 'بناء هيكل الصفحات', 'إضافة التفاعل', 'إدارة البيانات'],
...
export function QuizLesson({ lesson, isMobileMenuOpen, setIsMobileMenuOpen }: QuizLessonProps) {
    ...
    <div className="bg-white rounded-xl shadow-sm border-0 h-full">
      ...
      <div className="p-4 lg:p-6 space-y-6">
        {questions.map((q, index) => (
          <div key={q.id} className="border border-gray-100 rounded-xl p-4 lg:p-6 shadow-sm">
            <h4 className="font-semibold text-gray-900 mb-4 text-right">
              السؤال {index + 1}: {q.question}
            </h4>
            <div className="space-y-3">
              {q.options.map((option) => (
                <label key={option} className="flex items-center gap-3 p-3 border border-gray-100 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors">
                  <span className="text-right flex-1 font-medium text-gray-700">{option}</span>
                  <input
                    type="radio"
                    name={q.id}
                    value={option}
