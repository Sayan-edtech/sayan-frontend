...
import {
  ...
  List, ListOrdered, Link, Image, Type,
...
const FlowbiteTextEditor: React.FC<FlowbiteTextEditorProps> = ({
  ...
  const insertLink = () => {
    ...
    if (url && text) {
      executeCommand('insertHTML', `<a href="${url}" target="_blank" class="text-blue-600 underline hover:text-blue-800">${text}</a>`);
    }
  };
  ...
  const insertImage = () => {
    ...
    if (url) {
      executeCommand('insertHTML', `<img src="${url}" alt="${alt || 'صورة'}" class="max-w-full h-auto rounded-lg my-2" />`);
    }
  };
          ...
          {isPreview ? (
            ...
            <>
              <div
                ...
                className={`min-h-[150px] p-4 focus:outline-none ${error ? "border-red-500" : ""}`}
                style={{
                  outline: 'none',
                  direction: 'rtl',
                  textAlign: 'right',
