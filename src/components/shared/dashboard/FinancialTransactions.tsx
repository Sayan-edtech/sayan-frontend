...
import type { TransactionFilter, DateRange, Transaction } from "@/types/wallet";
...
const getAmountColor = (transaction: Transaction) => {
  ...
  if (transaction.change) {
    return transaction.change.startsWith("+")
      ? "text-green-600"
      : "text-red-600";
  }
...
export default function FinancialTransactions() {
  ...
  useEffect(() => {
    ...
    if (!tokens.accessToken || !tokens.refreshToken) {
      console.warn(
        "Authentication tokens are missing. User may need to log in again."
      );
    }
  }, [transactions, error]);
                ...
                >
                  ...
                  <TableCell className="text-right py-4">
                    <div className="text-gray-900 font-medium">
                      ﷼{transaction.after || transaction.balance || "-"}
                    </div>
                  </TableCell>
                  <TableCell className="text-right py-4">
                    <span
      ...
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-4">
          <div className="text-sm text-muted-foreground">
            ...
            {Math.min(currentPage * ITEMS_PER_PAGE, transactions.length)} من{" "}
