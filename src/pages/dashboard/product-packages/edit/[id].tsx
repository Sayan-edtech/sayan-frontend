...
// Sample package data based on real products
const samplePackage = {
  id: 1,
...
function EditProductPackage() {
  const navigate = useNavigate();
  const { id } = useParams<{ id: string }>();
  const [title, setTitle] = useState(samplePackage.title);
  const [description, setDescription] = useState(samplePackage.description);
  const [price, setPrice] = useState(samplePackage.price.toString());
  const [discountedPrice, setDiscountedPrice] = useState(samplePackage.discountedPrice?.toString() || "");
  const [selectedProducts, setSelectedProducts] = useState(samplePackage.products);
  const [imagePreview, setImagePreview] = useState(samplePackage.image);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("Updated Product Package Data:", {
        id,
        title,
        description,
      ...
      enhancedToast.success("تم تحديث الباقة بنجاح", {
        description: "تم حفظ التغييرات على الباقة"
      });
      
      // Navigate back to packages list
      navigate("/dashboard/product-packages");
    } catch (error) {
      enhancedToast.error("خطأ في تحديث الباقة", {
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-semibold text-gray-900">تعديل باقة المنتجات</h1>
              <p className="text-sm text-gray-600">تعديل الباقة: {samplePackage.title}</p>
            </div>
          </div>
        </div>
      </div>
